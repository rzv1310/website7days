...
const steps = [
  {
    ...
    step: "01",
    title: "Discutăm",
    description: "Înțelegem exact ceea ce ai nevoie și stabilim structura site-ului.",
    day: "Ziua 1",
  },
  {
    icon: Palette,
          ...
          {steps.map((step, index) => (
            <ScrollReveal key={step.step} delay={index * 0.08}>
              <div className={`relative text-center group h-full flex flex-col ${index < steps.length - 1 ? "mb-5 md:mb-0" : ""}`}>
                ...
                {/* Glass card */}
                <div className="rounded-2xl bg-[#2a1f0d]/75 backdrop-blur-md border border-white/10 px-6 py-6 shadow-lg flex-1 flex flex-col justify-center">
